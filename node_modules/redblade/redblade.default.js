    ...
    for (var j = 0; j < rules.length; j++) {
      ...
      if (rule) {
        ...
        */
        ...
        rule.indexOf(DEFAULTS) > -1 && defaults.push(field)
      }
...
var filter = function(schemaName, target, cb, options) {
  ...
  */
  ...
  var fields     = Object.keys(target)
    , _defaults_ = schema._defaults_
    , fieldLen   = fields.length + _defaults_.length
  ...
  for (var i = 0; i < fieldLen; i++) {
    ...
    //对末处理值来做处理
    if (i >= fields.length && typeof value != 'undefined') {
      continue
    }
    ...
    if (typeof rules == 'undefined') {
      ...
      var names = Object.keys(rules)  //require, min, max
  ...
  //处理不需要值的规则；如默认值
  for (i = 0; i < schema._defaults_.length; i++) {
    field = schema._defaults_[i]
