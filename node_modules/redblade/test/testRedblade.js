...
describe('Redblade 测试用例', function() {
  ...
  describe('测试有序集合的多条件联合查询 （即所有schema中定义的index/keywords均有权重）', function() {
    ...
    */
    ...
    it('Redblade: select 前后数据并比较', function(done) {
      ...
      //正序取的数据
      redblade.select('document', where, function(err, docs, len1) {

        //倒序取的数据
        redblade.select('document', where, function(err, revdocs, len2) {

          //中间的数据
          redblade.select('document', where, function(err, orderdocs, len3) {

            //应该有168条符合的元素
            assert.equal(len1, len2, len3, 168)
  ...
  describe('测试查询条件的索引均为无序集合的例子', function() {
    /*
    预定义schema
    */
    redblade.schema('people', {
        "uid"         : "id"
      , "first_name"  : "index('first_name')"
      , "last_name"   : ""
      , "gender"      : "index('gender')"
      , "hobby"       : "keywords('hobby')"
      , "joinTime"    : "int"
    })


    it('Redblade: 插入大量people数据(无序集合)', function(done) {
      var family  = '白毕卞蔡曹岑常车陈成程池邓丁范方樊费冯符傅甘高葛龚古关郭韩何贺洪侯胡华黄霍姬简江姜蒋金康柯孔赖郎乐雷黎李连廉梁廖林凌刘柳龙卢鲁陆路吕罗骆马梅孟莫母穆倪宁欧区潘彭蒲皮齐戚钱强秦丘邱饶任沈盛施石时史司徒苏孙谭汤唐陶田童涂王危韦卫魏温文翁巫邬吴伍武席夏萧谢辛邢徐许薛严颜杨叶易殷尤于余俞虞元袁岳云曾詹张章赵郑钟周邹朱褚庄卓'
        , given   = '一乙二十丁厂七卜人入八九几儿了力乃刀又三于干亏士工土才寸下大丈与万上小口巾山千乞川亿个勺久凡及夕丸么广亡门义之尸弓己已子卫也女飞刃习叉马乡丰王井开夫天无元专云扎艺木五支厅不太犬区历尤友匹车巨牙屯比互切瓦止少日中冈贝内水见午牛手毛气升长仁什片仆化仇币仍仅斤爪反介父从今凶分乏公仓月氏勿欠风丹匀乌凤勾文六方火为斗忆订计户'
        , hobby   = ['音乐', '画画', '运动', '读书', '唱歌', '跳舞', '驾驶', '外语', '钓鱼', '跑步', '养生', '游戏']

      for (var i = 0; i < 1000; i++) {
        var idx = Math.random() * hobby.length | 0
          , hob = hobby.slice(idx, idx + Math.random() * 4 | 0)

        redblade.insert('people', {
            uid         : 'u' + ((i + 1) * 10000)
          , first_name  : family.charAt(Math.random() * family.length)
          , last_name   : given.charAt(Math.random() * given.length)
          , gender      : i % 2 === 0 ? '男' : '女'
          , hobby       : hob
          , joinTime    : (i + 1000)
        })
      ...
      redblade.client.on("idle", function() {
        ...
        timer = setTimeout(function() {
          console.log('massive people inserted', arguments)
          done()
        }, 400)
      })
    })

    it('Redblade: 查询People', function(done) {
      redblade.select('people', { gender: '女', hobby: '唱歌' }, function(err, result1, len1) {
        redblade.select('people', { gender: '女', hobby: '唱歌' }, function(err, result2, len2) {
          console.log('result1 len1 result2 len2', result1.length, len1, result2.length, len2)

          assert.equal(result1.length, len1)
          assert.equal(len1, len2)
          assert.equal(result2[0].uid, result1[0].uid)

          //{ from: 0, to: 10 } 应该有11条数据
          assert.equal(result2.length, 11)
          done()
  ...
  describe('删除相关测试数据', function() {
    ...
    it('Redblade: 清除people相关测试数据', function(done) {
      for (var i = 0; i < 1000; i++) {
        redblade.remove('people', { uid : 'u' + ((i + 1) * 10000) })
      }

      redblade.client.del('hobby')
