  ...
  var Handler = {
    ...
    */
    ...
    , get: function(expression, handler, options) {
      if (!expression) {
        Logger.debug('get expression ignored');
      } else {
        var mapper = new Mapper(expression, handler, options);
        Handler.handlers.push(mapper);
      }
    ...
    , post: function(expression, handler, options) {
      if (expression && handler) {
        return this.get(expression, handler, options || 'qs');
      }
      return self;
    }
        ...
        render: function(tmplUrl, _model, outFn, isRawTmpl) {
          ...
          */
          var getFile = function(filePath, cb) {
            ...
            var cachedTemplate = templatePool[filePath]
            ...
            var updateCache = function(err, tmpl) {
              if (err) {
                ...
                templatePool[filePath] = tmpl;
                Logger.debug('update template cache', filePath);
              }
            };
          ...
          */
          var getInclude = function(tmpl, cb) {
            tmpl = (tmpl || '').replace(includeRegExp, function(fileStr) {
              ...
              var includePath = path.join(res.home || Settings.home, includeFile);
              getFile(includePath);
              return templatePool[includePath] || fileStr;
            });
          ...
          */
          ...
          var filePath = path.join(res.home || Settings.home, tmplUrl)

          isRawTmpl
            ? getInclude(filePath, render)
            : getFile(filePath, render)
        }
      , renderRaw: function(rawTmpl, model, outFn) {
          this.render(rawTmpl, model, outFn, true);
        }
    ...
    */
    ...
    var handlePath = function(phyPath) {
      fs.stat(phyPath, function(err, stat) {
        ...
        if (err) {
          Logger.debug(phyPath, 'not found')
          self.write404(res);
          return
        }
