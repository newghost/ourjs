...
WebSvr中有一个模板缓存对象，为了提升响应速度，会直接获取缓存中的模板，首次启动ourjs时，此缓存对象为空。在 deubg = true (config.js)时，请求完成后会自动更新这些缓存模板；而在 debug = false 时，缓存对象一旦获取，将不再更新，即不产生I/O读写操作（有些厂商会对I/O操作收费），此时对模板的修改，需要重启ourjs进程应用。
